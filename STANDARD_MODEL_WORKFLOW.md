...
- [x] **校准曲线 + Brier 分数**(已内置 `--eval`)。swing 实测十分位最大偏离 ~0.12 → **概率不可直接用**,
  只可当排序器取 top;跨模型版本 score 绝对值也不可比(见 Phase 2 pooled 说明),故档位门槛必须随模型一起存盘。
...
Phase 7 sign-off:1无泄露✅ 2 AUC/lift✅ 3 gap 0.113✅ 4校准△(仅排序用) 5多seed✅ → **可上线,概率不可直接解读**。
