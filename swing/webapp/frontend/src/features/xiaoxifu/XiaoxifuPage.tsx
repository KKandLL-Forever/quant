...
interface StratCfg { key: string; label: string; hasKL: boolean; kicker: string; desc: string }
    ...
    <div>
      ...
      {!loading && data && (
        <>
          ...
          <Card size="small" title={`调仓动作(共 ${data.rebalances.length} 次${cfg.hasKL ? `,每 ${data.params.K} 交易日一次` : ',每日'},最新在前)`}>
