...
8) val_method: **主锚已由决策树选定(见「★估值主锚」),你只做两件事**:①用该主锚 + 「同业可比」+「估值工具箱/前瞻PEG」里对应的数字,给一句"低估/合理/高估"判断 + 关键数字;②**不得改用其它方法**。
   各主锚怎么读数:PE(前瞻)→ **优先用「同业可比与行业」里『研报可比公司估值表』的本股前瞻PE 对比同业前瞻PE中位**(口径一致);无研报表时才用 TTM PE + 前瞻PEG;叠PEG看增速匹配。**别把当年/TTM PE 与前瞻PE混用**。PB-ROE/PB → 合理PB≈ROE匹配,跌破净资产=底部信号。周期中枢PE → **用正常化利润**(忌当期PE:顶部PE最低=陷阱),叠PB判底;**底部高PB/高PE是给未来盈利定价、别据此判顶**。股息率 → 看分红可持续性与股息率高低。EV/EBITDA → 剔折旧负债后对比同业。
