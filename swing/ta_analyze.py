...
def analyze_live(code, date, on_event, analysts=("market", "news")):
    ...
    for chunk in ta.graph.stream(init, **args):
        ...
        need = [k for a, k in (("market", "market_report"), ("news", "news_report")) if a in analysts]
        if need and all(final.get(k) for k in need):   # 分析师报告一齐,即停:辩论/研究经理/交易员的输出没人用,不跑省 DeepSeek(~6次调用)
            return final
    return final
...
def _business_prompt(code, date=None):
    ...
    try:
        import valuation
        _anc = valuation.pick_anchor(tscode)
        anchortxt = f"{_anc['anchor']}(原型:{_anc['archetype']};依据:{_anc['reason']})"
    except Exception:
        anchortxt = "PE(动态扣非)(默认)"
    info = (f"主营:{r['main_business']}\n简介:{str(r['introduction'])[:600]}\n申万行业:{sw}\n"
            ...
            f"★估值主锚(已由确定性决策树选定,直接采用,勿另择方法):{anchortxt}\n"
...
7) val_type: **直接填「★估值主锚」那行给出的原型**(资产型/成长型/现金流型/周期成长型/金融/纯预期),不要自己另判。
8) val_method: **主锚已由决策树选定(见「★估值主锚」),你只做两件事**:①用该主锚 + 「同业可比」+「估值工具箱/前瞻PEG」里对应的数字,给一句"低估/合理/高估"判断 + 关键数字;②**不得改用其它方法**。
   各主锚怎么读数:PE(前瞻)→ **用「同业可比与行业」里那行『前瞻PE(主口径)』:本股前瞻PE 对比同业前瞻PE中位**(两者都是 总市值÷券商一致预测净利,同口径同日期);该行缺失才退 TTM PE + 前瞻PEG;叠PEG看增速匹配。**同业TTM中位只作辅助、不得与前瞻PE混用**(实测两者可差一倍)。**别把当年/TTM PE 与前瞻PE混用**。PB-ROE/PB → 合理PB≈ROE匹配,跌破净资产=底部信号。周期中枢PE → **用正常化利润**(忌当期PE:顶部PE最低=陷阱),叠PB判底;**底部高PB/高PE是给未来盈利定价、别据此判顶**。股息率 → 看分红可持续性与股息率高低。EV/EBITDA → 剔折旧负债后对比同业。
   **主锚=「无法估值」时**:val_method 直接写"纯预期驱动,相对法失效,暂无法估值",不要硬凑。
