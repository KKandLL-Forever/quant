"""
ml_train_1to2_v1.py — XGBoost 训练：一板→二板 晋升概率模型 v3（复用 2lb_v3 的 34 特征）

信号：limit_list_d.limit_times = 1（首板；不筛量能、不筛 ST、剔除 创业/科创/北交所）
label：T+1 pct_chg >= 9.8（次日封板=2 板成功，简单二分类）
特征：ml_features_1to2_v1.FEATURE_COLS（1进2 独立，35 个：2lb_v3 的 34 - 3 死特征 + 4 技术面因子）

产出：
  model/xgb_1lb_2lb_v1.pkl
  model/shap_summary_1to2_v1.png
  model/feature_matrix_1to2_v1.csv
...
MODEL_PATH = os.path.join(MODEL_DIR, "xgb_1lb_2lb_v1.pkl")
SHAP_IMG   = os.path.join(MODEL_DIR, "shap_summary_1to2_v1.png")
...
def _get_signals():
    """扫描 1 板信号（limit_times=1，非 ST，剔除 创业/科创/北交所），按日期缓存。"""
...
def _evaluate(model, X_test, y_test, feat_cols, test_df):
    ...
    if "next_pct" in test_df.columns and "next2_pct" in test_df.columns:
        ...
        print(f"\n  分量诊断：")
        print(f"    T+1 涨停率（2 板成功）          = {d['lb_t1'].mean():.2%}")
        m = d[d["lb_t1"] == 1]
        if len(m) > 0:
            print(f"    2 板成功后 T+2 仍正收益比例    = {m['lb_t2pos'].mean():.2%}  ({len(m)} 个)")
...
def train():
    ...
    feat_csv = os.path.join(MODEL_DIR, "feature_matrix_1to2_v1.csv")
