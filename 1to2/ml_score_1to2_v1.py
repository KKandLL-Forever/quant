"""
ml_score_1to2_v1.py — 每日盘后 首板个股晋级 2 板成功率打分 v1（用 ml_features_1to2_v1（34 特征：含 bias2/rsi12/topdays））

特征含 4 个集合竞价（开盘抢筹量/竞价异动/开盘资金/尾盘竞价占比），
模型用 xgb_1lb_2lb_v1.pkl。需先用 cache_tushare.py 补齐 stk_auction_o/c 数据。

—— 以下为 v2 原始说明 ——
用法：

用法：
  python 1to2/ml_score_1to2_v1.py                 # 最新交易日
  python 1to2/ml_score_1to2_v1.py --date 20260508 # 指定历史日期复盘
  python 1to2/ml_score_1to2_v1.py --top 20        # 上限条数（候选已先按 Top 1% 过滤）
  每次执行自带 SHAP 因子分解 + 市场状态横幅，输出 HTML 并自动打开
  候选榜只展示 proba ≥ Top 1% 阈值的个股（无 Top 1% 候选则不出 HTML，仅终端打印 proba）

模型：
  分类模型 (xgb_1lb_2lb_v1.pkl)：
    proba = P(T+1 pct_chg >= 9.8)  即 P(2 板成功，即首板晋级2板)
    用途：排序 + 信号过滤

  历史 OOS 表现（2024+ 测试集，34 特征 v1）：
    基线晋级率 19.50%  AUC 0.642
    Top 1%  胜率 74.52% (proba≥0.8617, n=259)
    Top 5%  胜率 45.76%
    Top 10% 胜率 38.10%
    Top 20% 胜率 33.03%
...
依赖：
  - 模型: first10/model/xgb_1lb_2lb_v1.pkl
...
def _load_model():
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            f"模型不存在: {MODEL_PATH}\n请先运行 python 1to2/ml_train_1to2_v1.py")
...
.tip{{font-size:11px;color:#999;margin-left:6px}}
    ...
    html.append(f"<div class='meta' style='margin-top:24px'>"
                ...
                f"模型路径：{escape(MODEL_PATH)} · 首板→2板 v1</div>")
...
def score(trade_date, top_n):
    ...
    title = f"【首板→2板·v1·含竞价】晋升概率打分 {trade_date}"
    ...
    out_html = os.path.join(os.path.dirname(__file__), "html_output", f"ml_score_1to2_v1_{trade_date}.html")
