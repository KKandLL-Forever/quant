...
本文件自包含：信号/特征/模型解析 + 四套回测引擎全部内联（原 sim_2lb_2026/ranked/tier/dynamic 已并入）。

口径（智能止盈）：买点=2板次日开盘；卖出=炸板次日开盘 / 连板断板当日开盘。含「对续板完美预判」
...
产出：first10/html_output/sim_2lb_all_{start}_{end}_{ver}.html（自动打开）
"""
...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "ml_v2"))
sys.stdout.reconfigure(encoding="utf-8")

import screen_2lb_top1_t7_v2 as S
...
def _resolve_model(version: str):
    """按版本返回 (model, feat_cols, tiers, build_feature_matrix)；v2/v3 差异仅在 pkl 与特征模块。"""
...
def _load_trades(start: str, end: str, tier_pct: int,
                 version: str = "v3") -> tuple[pd.DataFrame, float]:
    ...
    tr = con.execute("""
        ...
        SELECT ts_code, d2_str trade_date, boards,
          ...
          MAX(CASE WHEN boards<=2 AND off=2 THEN strftime(td,'%Y%m%d')
                   WHEN boards>=3 AND off=boards-1 THEN strftime(td,'%Y%m%d') END) sell_date,
          MAX(CASE WHEN boards<=2 AND off=2 THEN open
                   WHEN boards>=3 AND off=boards-1 THEN open END) sell_price
...
def build_tier(start, end, per_cap, version="v3"):
...
def build_dynamic(df, capital, version="v3"):
    """策略4：单账户动态仓位。"""
...
def render_html(strategies, span, start, end, ver, cutoffs) -> str:
    ...
    header = (
        ...
        '<span><b>proba</b>＝模型预测「该 2 板个股次日继续涨停、晋级 3 板」的概率'
        '（0~1，越高越强）</span>'
...
def main():
    ...
    out = os.path.join(os.path.dirname(__file__), "html_output",
                       f"sim_2lb_all_{args.start}_{args.end}_{ver}.html")
