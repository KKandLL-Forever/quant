"""
ml_score_2lb_v3.py — 每日盘后 2 板个股晋级 3 板成功率打分 v3（v2 + 集合竞价特征）

v3 在 v2 基础上新增 4 个集合竞价特征（开盘抢筹量/竞价异动/开盘资金/尾盘竞价占比），
模型用 xgb_2lb_3lb_v3.pkl。需先用 cache_tushare.py 补齐 stk_auction_o/c 数据。

—— 以下为 v2 原始说明 ——
ml_score_2lb_v2.py — 每日盘后 2 板个股晋级 3 板成功率打分

用法：
  python first10/ml_score_2lb_v2.py                  # 最新交易日
  python first10/ml_score_2lb_v2.py --date 20260508  # 指定历史日期复盘
  python first10/ml_score_2lb_v2.py --top 20         # 上限条数（候选已先按 Top 10% 过滤）
...
def _load_model():
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            f"模型不存在: {MODEL_PATH}\n请先运行 python first10/ml_train_2lb_v3.py")
...
def score(trade_date, top_n):
    ...
    title = f"【2板→3板·v3·含竞价】晋升概率打分 {trade_date}"
    ...
    out_html = os.path.join(os.path.dirname(__file__), "html_output", f"ml_score_2lb_v3_{trade_date}.html")
