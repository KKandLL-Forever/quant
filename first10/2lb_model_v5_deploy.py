"""
2lb_model_v5_deploy.py — 用 v5 评估锁定的轮数 + 全部历史数据，训练 2板→3板【实盘部署模型】

与 ml_train_2lb_v5.py（评估）的分工：
  评估(v5)   : train ≤2023 / val 2024 / test 2025~ —— 留干净测试集，诚实估分、定轮数
  部署(本文件): 关早停、轮数锁死成 v5 折4 的 best_iter，喂 ≤今天全部数据 —— 出实盘最强模型
...
特征：ml_features_2lb_v3（31 特征，与 v5 一致）
轮数：默认读 v5 pkl 折4 的 best_iter；--rounds 可手动覆盖
分位档切点：直接沿用 v5 pkl 的 tiers（2022~2026 全样本外合并标定，跨熊牛）

产出：
  model/xgb_2lb_3lb_v5_deploy.pkl —— 实盘打分用（结构同 v5：model/feature_cols/tiers/meta）

用法：
  python first10/2lb_model_v5_deploy.py
  python first10/2lb_model_v5_deploy.py --rounds 60   # 手动指定轮数
"""
...
V5_PATH    = os.path.join(MODEL_DIR, "xgb_2lb_3lb_v5.pkl")
DEPLOY_PATH = os.path.join(MODEL_DIR, "xgb_2lb_3lb_v5_deploy.pkl")
...
def _get_signals():
    """扫描全部 2 板信号（limit_times=2，非 ST，剔除 创业/科创/北交所），按日期缓存。"""
...
def _load_v5():
    """读 v5 评估 pkl，返回 (折4 best_iter, tiers)。"""
    if not os.path.exists(V5_PATH):
        raise RuntimeError(f"未找到 v5 评估模型 {V5_PATH}，请先跑 ml_train_2lb_v5.py")
    with open(V5_PATH, "rb") as f:
        b = pickle.load(f)
    folds = b.get("meta", {}).get("folds")
    if not folds:
        raise RuntimeError("v5 pkl 缺 meta.folds，请重训 ml_train_2lb_v5.py（已加 best_iter 记录）。")
...
def train(rounds_override=None):
    ...
    t0 = time.time()
    locked_rounds, tiers = _load_v5()
    rounds = rounds_override or locked_rounds
    print(f"=== 部署训练 ===")
    print(f"  锁定轮数：{rounds}（来源：{'手动 --rounds' if rounds_override else 'v5 折4 best_iter='+str(locked_rounds)}）")
    ...
    meta = {"rounds": int(rounds),
            ...
            "tier_calib": "沿用 v5（2022~2026 全样本外合并标定）"}
...
if __name__ == "__main__":
    ...
    ap.add_argument("--rounds", type=int, default=None, help="手动指定轮数，覆盖 v5 折4 best_iter")
    args = ap.parse_args()
    train(args.rounds)
