"""
ml_score_2lb_v2.py — 每日盘后 2 板个股晋级 3 板成功率打分

用法：
  python first10/ml_v2/ml_score_2lb_v2.py                  # 最新交易日
  python first10/ml_v2/ml_score_2lb_v2.py --date 20260508  # 指定历史日期复盘
  python first10/ml_v2/ml_score_2lb_v2.py --top 20         # 上限条数（候选已先按 Top 10% 过滤）
  每次执行自带 SHAP 因子分解 + 市场状态横幅，输出 HTML 并自动打开
  候选榜只展示 proba ≥ Top 10% 阈值的个股（不足则只显示达标的）

模型：
  分类模型 (xgb_2lb_3lb_v2.pkl)：
    proba = P(T+1 pct_chg >= 9.8)  即 P(3 板成功)
    用途：排序 + 信号过滤

  历史 OOS 表现（2024+ 测试集）：
    基线正例率 36.00%  AUC 0.6921
    Top 1% 胜率 100.00% (48/48，样本小)
    Top 5% 胜率 81.48% (2.26x lift)
    Top 10% 胜率 75.98% (2.11x lift)
...
依赖：
  - 模型: first10/model/xgb_2lb_3lb_v2.pkl
...
def _load_model():
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            f"模型不存在: {MODEL_PATH}\n请先运行 python first10/ml_v2/ml_train_2lb_v2.py")
...
.tip{{font-size:11px;color:#999;margin-left:6px}}
    ...
    html.append(f"<div class='meta' style='margin-top:24px'>"
                ...
                f"模型路径：{escape(MODEL_PATH)} · 2板→3板 v2</div>")
...
def score(trade_date, top_n):
    ...
    title = f"【2板→3板·v2】晋升概率打分 {trade_date}"
    ...
    out_html = os.path.join(os.path.dirname(os.path.dirname(__file__)), "html_output", f"ml_score_2lb_v2_{trade_date}.html")
