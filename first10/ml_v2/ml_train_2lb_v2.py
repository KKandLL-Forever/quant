"""
ml_train_2lb_v2.py — XGBoost 训练：二板→三板 晋升概率模型

信号：limit_list_d.limit_times = 2（不筛量能、不筛 ST、剔除 创业/科创/北交所）
label：T+1 pct_chg >= 9.8（次日封板=3 板成功，简单二分类）
特征：ml_features_2lb_v2.FEATURE_COLS（32 个 = 复用 26 + 新增 6，含 has_cb 可转债标记）

产出：
  model/xgb_2lb_3lb_v2.pkl
  model/shap_summary_2lb_v2.png
  model/feature_matrix_2lb_v2.csv
...
MODEL_PATH = os.path.join(MODEL_DIR, "xgb_2lb_3lb_v2.pkl")
SHAP_IMG   = os.path.join(MODEL_DIR, "shap_summary_2lb_v2.png")
...
def train():
    ...
    from ml_features_2lb_v2 import build_feature_matrix, FEATURE_COLS
    ...
    feat_csv = os.path.join(MODEL_DIR, "feature_matrix_2lb_v2.csv")
