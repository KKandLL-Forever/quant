"""
ml_train_1to3lb_v2.py — XGBoost 训练：首板直接预测 3 板（端到端）

信号：全部首板（limit_times=1，非ST，非创业/科创/北交所，不筛量能、不剔除一字板）
label：(T+1 pct_chg >= 9.8) AND (T+2 pct_chg >= 9.8)  即「双涨停」，T 日盘后预测连续 2 天封板
复用 ml_features_v2 的特征工程（26 个特征 + top10），在训练时重写 label。
注意：不引入 d1_* 类特征（那是 2 板专属，T 日时 T+1 尚未发生）。

产出：
  model/xgb_1to3lb_v2.pkl
  model/shap_summary_1to3lb_v2.png
  model/feature_matrix_1to3lb_v2.csv

正例率预估 7-8%（远低于首板→2板的 12%、2板→3板的 36%）。
模型上限预期 AUC 0.55-0.58、Top 5% 胜率 15-20%（端到端损失 T+1 信息，理论上劣于链式）。
"""
...
MODEL_PATH = os.path.join(MODEL_DIR, "xgb_1to3lb_v2.pkl")
SHAP_IMG   = os.path.join(MODEL_DIR, "shap_summary_1to3lb_v2.png")
...
def _get_signals():
    """扫描全部首板信号（缓存复用 ml_train_v2 同款 pkl）。"""
    ...
    try:
        ...
        cache_path = os.path.join(CACHE_DIR, f"signals_1lb_all_{latest}.pkl")
        ...
        t = time.time()
        print(f"扫描全部首板信号（limit_times=1，非 ST，非创业/科创/北交所）...", flush=True)
...
def _evaluate(model, X_test, y_test, feat_cols, test_df):
    ...
    if "next_pct" in test_df.columns and "next2_pct" in test_df.columns:
        ...
        d["lb_t2"] = (d["next2_pct"] >= 9.8).astype(int)
        print(f"\n  分量诊断：")
        print(f"    T+1 涨停率                      = {d['lb_t1'].mean():.2%}")
        print(f"    T+1 涨停 后 T+2 也涨停（条件率）  = "
              f"{d[d['lb_t1']==1]['lb_t2'].mean():.2%}  ({int(d['lb_t1'].sum())} 个 T+1 涨停)")

    return proba


def train():
    """训练入口：装载信号 → 构建特征 → 重写 label → 训练 → 评估 → SHAP。"""
    ...
    print("\n=== 构建特征矩阵（复用 ml_features_v2）===")
    feat_df = build_feature_matrix(signal_df)
    print(f"特征矩阵：{feat_df.shape}")

    feat_df = feat_df.dropna(subset=["next_pct", "next2_pct"]).copy()
    feat_df["label"] = ((feat_df["next_pct"] >= 9.8)
                       & (feat_df["next2_pct"] >= 9.8)).astype(float)
    print(f"重写 label（T+1 涨停 AND T+2 涨停 = 3 板成功）：\n{feat_df['label'].value_counts(dropna=False)}")
    ...
    if shap is not None:
        try:
            ...
            for r, (f, v) in enumerate(ranking, 1):
                print(f"  {r:2d}. {f:<26s}  {v:.4f}")
        except Exception as e:
            print(f"SHAP 失败: {e}")

    feat_csv = os.path.join(MODEL_DIR, "feature_matrix_1to3lb_v2.csv")
