"""
screen_2lb_top1_t7_v2.py — 2板→3板模型 分位档历史选股 + T+7 收益统计

用法：
  python first10/ml_v2/screen_2lb_top1_t7_v2.py                      # 默认 Top 1%
  python first10/ml_v2/screen_2lb_top1_t7_v2.py --tier 5             # Top 5%
  python first10/ml_v2/screen_2lb_top1_t7_v2.py --tier 10 --start 20250101
  python first10/ml_v2/screen_2lb_top1_t7_v2.py --tier 10 --tier-top 5  # 只取 Top10%~Top5% 之间
  python first10/ml_v2/screen_2lb_top1_t7_v2.py --proba 0.60 --proba-hi 0.66  # 手动区间切点

口径：
  信号日 = 2板日（模型于 2板日盘后打分）
  T = 首板日 = 2板日的前一交易日；T+7 = 2板日 + 6 个交易日
  收益口径（均以 2板次日开盘为买点）：
    ret_real（主口径·智能止盈）：
      止步2板(炸板) → 次日(2板+2)开盘卖；连板 → 持有到断板当日开盘价卖出
      （默认对续板有完美预判，连板就一直拿；断板那天在开盘价离场）
    ret_t3_buy / ret_from_buy（参考·固定持有）：close(2板+2 / 2板+6) / 买点 - 1
  顶部诊断（资金曲线/回撤/去头部）均基于 ret_real。
  筛选：proba >= 所选分位档切点（从 pkl tiers 自动读取，--tier 选 1/5/10/20）

产出：
  first10/html_output/screen_2lb_top{tier}_t7_v2.html（自动打开）
"""
...
MODEL_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "model", "xgb_2lb_3lb_v2.pkl")
DUCK_PATH  = _ENV.get("LOCAL_DUCKDB_PATH", "stock_data_tushare.duckdb")
OUT_HTML   = os.path.join(os.path.dirname(os.path.dirname(__file__)), "html_output", "screen_2lb_top1_t7_v2.html")
...
def _attach_prices(con, signal_df: pd.DataFrame) -> pd.DataFrame:
    """给 2板信号附加 首板收盘(T)、2板次日开盘(买点)、T+7 收盘 及对应日期。"""
...
def _attach_boards(con, signal_df: pd.DataFrame) -> pd.DataFrame:
    """给每个 2板信号附加其所在连板序列的最终高度（如 4 = 走到 4连板后断板）。"""
...
def _attach_realized(con, signal_df: pd.DataFrame) -> pd.DataFrame:
    """按智能止盈口径算实际卖价：炸板(止步2板)→次日开盘卖；连板→持有到断板当日开盘卖。"""
...
<title>2板→3板 Top1% 选股 T+7 收益</title>
...
var columns=[
  ...
  {title:"首板日(T)",dataIndex:"t_date_1lb",width:104,
...
function App(){
  return e("div",null,
    e("h1",null,"2板→3板模型 · "+P.tier_label+" 档（proba ≥ "+P.proba_thr.toFixed(4)+"）历史选股"),
    ...
    e("div",{className:"grid"},
      ...
      card("最终连板高度分布","「2连板」= 止步 2板、没能开出 3板 · 样本 "+P.boards_n+" 只",
    ...
    e("div",{className:"meta"},"「智能止盈」收益口径：买点 = 2板次日开盘；止步2板(炸板) → 次日开盘卖出；晋级连板 → 一直持有，到断板当日开盘价卖出（默认对续板有完美预判）。"),
...
def run(start: str, tier_pct: int = 1, proba_thr: float = None,
    ...
    print(f"=== 扫 2 板信号（{start} 起）===")
    from ml_train_2lb_v2 import _get_signals
    sig = _get_signals()
    sig = sig[sig["trade_date"] >= start].reset_index(drop=True)
    print(f"  2 板信号数：{len(sig)}")

    from ml_features_2lb_v2 import build_feature_matrix
    ...
    out_html = os.path.join(os.path.dirname(os.path.dirname(__file__)), "html_output",
                            f"screen_2lb_top{tier_pct}_t7_v2.html")
    ...
    if len(validb):
        print(f"  T+7 从2板次日开盘买：均 {validb.mean()*100:+.2f}%  "
