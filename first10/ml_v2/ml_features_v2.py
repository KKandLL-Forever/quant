"""
ml_features_v2.py — 首板小量信号特征工程（v2 标签 + M4 特征调整）

label：
  (T+1 pct_chg >= 9.8) AND (T+2 pct_chg > 0)
  即 次日封板 + 第三日有溢价（>0%）

进出场假设：
  T 日盘后选股 → T+1 开盘买 → T+2 收盘卖出

M4 特征改动（详见 FEATURE_COLS 定义处）：
  减 5：close_strength / vol_ratio_20 / gap_up / sm_ratio / market_lu_count
  加 3：market_max_lianban / market_2lb_rate_ma5 / market_idx_dist_h60
  转换：pe_ttm → pe_ttm_log，pb → pb_log，net_mf_ratio → net_mf_ratio_log

M5 追加（top10 流通股东）：
  加 1：top10_float_ratio_sum（前十大流通股东累计占流通股比，PIT 安全：取 ann_date < sig_date 的最近一期）

依赖：
  - DuckDB 表 market_state（由 cache_tushare.py --update 时自动重建）
  - DuckDB 表 top10_floatholders（由 cache_tushare.py 拉取，季度披露）
"""
  ...
  signal_stats AS (
    SELECT
      ...
      CASE WHEN seq_num_before > 0
        THEN SUM(CASE WHEN next_pct >= 9.8 AND next2_pct > 0 THEN 1 ELSE 0 END)
...
SELECT
  ...
  CASE WHEN db.pe_ttm IS NULL THEN NULL
       ...
       ELSE -ln(1 + (-db.pe_ttm))
  END                                                                      AS pe_ttm_log,
  CASE WHEN db.pb IS NULL OR db.pb <= 0 THEN NULL
       ELSE ln(db.pb)
  END                                                                      AS pb_log,
  ...
  CASE
    WHEN d.next_pct IS NULL OR d.next2_pct IS NULL THEN NULL
    WHEN d.next_pct >= 9.8 AND d.next2_pct > 0     THEN 1.0
    ELSE 0.0
  END                                                                      AS label,
...
def build_feature_matrix(signal_df: pd.DataFrame, stock_dict=None,
    ...
    """构建特征矩阵；require_label=True 训练用（要求 next_pct 与 next2_pct），False 打分用。"""
    if signal_df.empty:
        return pd.DataFrame()
    ...
    sql = _FEATURE_SQL.format(
        label_filter="WHERE d.next_pct IS NOT NULL AND d.next2_pct IS NOT NULL"
                     if require_label else ""
    )

    t0 = time.time()
    con = _duckdb.connect(_DUCKDB_PATH, read_only=True)
    print(f"  [features_v2] DuckDB connect: {time.time()-t0:.2f}s", flush=True)
    try:
        t1 = time.time()
        ...
        con.execute("""
            ...
            FROM signals_raw
        """)
        print(f"  [features_v2] register signals ({len(sig)} rows): {time.time()-t1:.2f}s", flush=True)

        t2 = time.time()
        df = con.execute(sql).df()
        print(f"  [features_v2] SQL execute + fetch ({len(df)} rows): {time.time()-t2:.2f}s", flush=True)
    finally:
        con.close()

    print(f"  [features_v2] total: {time.time()-t0:.2f}s", flush=True)
    return df
