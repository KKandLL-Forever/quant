"""
ml_features_2lb_v3.py — 二板→三板信号特征工程 v3（在 v2 基础上 + 集合竞价特征）

v3 新增集合竞价特征（信号日 T 盘后可得；竞价 vol 单位=股、daily vol=手；竞价 amount=元、daily amount=千元，已做单位换算）：
...
FEATURE_COLS = [
    ...
    "d1_lu_time_min", "d1_fd_amount_ratio", "d1_vol_ratio_5",
    "gap_2lb",
...
FEATURE_CN = {
    ...
    "intraday_range": "当日振幅",
    "lu_time_min": "2板封板时间(早=强)",
    "fd_amount_ratio": "2板封单额/流通市值",
    ...
    "d1_lu_time_min": "首板(T-1)封板时间",
    "d1_fd_amount_ratio": "首板(T-1)封单/流通市值",
    "d1_vol_ratio_5": "首板(T-1)量比(爆量首板)",
    "gap_2lb": "2板高开幅度(开盘/首板收盘-1)",
...
def build_feature_matrix(signal_df: pd.DataFrame, stock_dict=None,
    ...
    try:
        ...
        con.execute("""
            ...
            FROM signals_raw
        """)
        print(f"  [features_2lb_v3] register signals ({len(sig)} rows): {time.time()-t1:.2f}s", flush=True)

        t2 = time.time()
        df = con.execute(sql).df()
        print(f"  [features_2lb_v3] SQL execute + fetch ({len(df)} rows): {time.time()-t2:.2f}s", flush=True)
    finally:
        con.close()

    print(f"  [features_2lb_v3] total: {time.time()-t0:.2f}s", flush=True)
    return df
