"""
ml_score_2lb_v5.py — 每日盘后 2 板个股晋级 3 板成功率打分 v5（实盘部署模型）

用 xgb_2lb_3lb_v5_deploy.pkl（31 特征，特征同 ml_features_2lb_v3）：该模型由
2lb_model_v5_deploy.py 产出——锁定 v5 评估的轮数、喂 ≤今天全部数据训练，是实盘最强模型。
分位档切点沿用 v5（2022~2026 全样本外合并标定，跨熊牛，比训练集分位更保守、更耐用）。
需先用 cache_tushare.py 补齐 stk_auction_o/c 数据，并先跑 2lb_model_v5_deploy.py 生成部署 pkl。

proba = P(T+1 pct_chg >= 9.8)，即 P(3 板成功)，用于排序 + 信号过滤。

用法：
  python first10/ml_score_2lb_v5.py                  # 最新交易日
  python first10/ml_score_2lb_v5.py --date 20260508  # 指定历史日期复盘
  python first10/ml_score_2lb_v5.py --top 20         # 上限条数（候选已先按 Top 10% 过滤）
  每次执行自带 SHAP 因子分解 + 市场状态横幅，输出 HTML 并自动打开
  候选榜只展示 proba ≥ Top 10% 阈值的个股（不足则只显示达标的）

信号过滤条件：
  - limit_list_d.limit_type='U' AND limit_times=2     2 板
  - 非 ST，排除创业板/科创板/北交所

依赖：
  - 模型: first10/model/xgb_2lb_3lb_v5_deploy.pkl（先跑 2lb_model_v5_deploy.py）
...
MODEL_PATH = os.path.join(os.path.dirname(__file__), "model", "xgb_2lb_3lb_v5_deploy.pkl")
...
def _print_tier_summary(trade_date: str, rows, thresholds) -> None:
    ...
    if not any_hit:
        print("（今日无 Top 10% 及以上达标候选）")

FACTOR_CN = {
    "pct_chg":              ("涨跌幅",              "信号日涨跌幅（2板≈10%）"),
    "vol_ratio_5":          ("5日量比",             "当日量 / 5日均量"),
    "intraday_range":       ("当日振幅",            "(高-低)/收盘"),
    "pre_days_limit":       ("前5日涨跌停数",       "前5日内|pct_chg|≥9.8 的天数"),
    "lu_time_min":          ("2板封板时间",         "首封距9:30分钟数（越早封越强）"),
    "fd_amount_ratio":      ("2板封单/流通市值",    "封单金额/流通市值（越大封板越硬）"),
    "open_times":           ("2板炸板次数",         "0=稳封"),
...
def _load_model():
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            f"模型不存在: {MODEL_PATH}\n请先运行 python first10/2lb_model_v5_deploy.py")
...
def _render_html(title, trade_date, n_total, regime_info, market_state,
                 ...
                 proba_header="3板成功概率", footer_label="2板→3板 v5·部署",
                 model_path=None) -> str:
...
.tip{{font-size:11px;color:#999;margin-left:6px}}
    ...
    html.append(f"<div class='meta'>交易日 {trade_date} · 2 板信号 {n_total} 只 · "
    ...
    html.append("<h2>候选 Top 榜（仅 Top 10% 及以上）</h2>")
    ...
    html.append(f"<div class='meta' style='margin-top:24px'>"
                ...
                f"模型路径：{escape(model_path or MODEL_PATH)} · {escape(footer_label)}</div>")
    html.append("</body></html>")
    return "".join(html)


def score(trade_date, top_n):
    """主流程：取 2 板信号 → 模型预测 → 排序 → SHAP → HTML。"""
    ...
    title = f"【2板→3板·v5·部署】晋升概率打分 {trade_date}"

    html = _render_html(title, trade_date, n_signals, regime_info, market_state,
                        rows, sv, feat_cols, base or 0.0, top[feat_cols],
                        thresholds, hit_rates, auc)

    out_html = os.path.join(os.path.dirname(__file__), "html_output", f"ml_score_2lb_v5_{trade_date}.html")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="2 板→3 板晋升概率打分（HTML 报告）")
