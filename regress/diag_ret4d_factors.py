"""
diag_ret4d_factors.py — 为「首板 4 天超额收益回归」批量筛单因子。

只读诊断，不写库、不改模型。流程：
  1. 取全部首板信号 + 4 天超额收益 label（口径同 ml_train_ret4d_v1：T+1 开盘买 / T+5 收盘卖 / 减中证1000）
  2. 在信号日 T0（PIT 安全）从 daily / daily_basic / moneyflow / cyq_perf / stk_factor_pro 构造 ~25 个候选因子
  3. 算每个因子对 label 的单因子 Spearman IC（全样本 + 按年），按 |IC| 排序

候选都是 T0 收盘即可知的量，无未来函数。用法：
  python regress/diag_ret4d_factors.py
"""
...
_SIGNAL_SQL = """
SELECT l.ts_code, strftime(l.trade_date, '%Y%m%d') AS trade_date
FROM limit_list_d l
LEFT JOIN stock_st st ON st.ts_code = l.ts_code AND st.trade_date = l.trade_date
WHERE l.limit_type='U' AND l.limit_times=1
  AND l.ts_code NOT LIKE '688%' AND l.ts_code NOT LIKE '30%' AND l.ts_code NOT LIKE '%.BJ'
...
CN = {"mom_5": "5日动量", "mom_10": "10日动量", "mom_20": "20日动量", "vol_20": "20日波动率",
      ...
      "vr5": "5日量比", "turn_accel": "量能加速(5日/20日均量)", "pct_chg": "信号日涨幅",
      "turnover_rate_f": "自由流通换手", "volume_ratio": "量比(daily_basic)", "pe_ttm": "PE_TTM",
      "pb": "PB", "ps_ttm": "PS_TTM", "dv_ratio": "股息率", "circ_mv_log": "流通市值(log)",
      "free_ratio": "自由流通占比", "net_mf_ratio": "净主力额/成交额", "sm_net_ratio": "小单净额/成交额(散户)",
      "elg_net_ratio": "特大单净额/成交额", "inst_net_ratio": "大+特大单净额/成交额(机构)",
...
def main():
    ...
    print(f"首板信号 {len(sig)}（{sig['trade_date'].min()}~{sig['trade_date'].max()}）")
    ...
    print(f"有效样本 {len(df)}  label 均值 {df['label'].mean():+.4f}  正例率 {(df['label']>0).mean():.1%}\n")
    ...
    for c in CANDIDATES:
        ...
        cov = m.mean()
        rows.append((c, ic_all, np.nanmean(yic), np.nanstd(yic), cov, yic))
    ...
    print(f"\n逐年顺序：{years}")
    print("提示：|全样本IC|>0.03 且 年均IC 同号、年IC标准差小 = 稳健可用；符号正=值越大越看涨。")
